...
class TestDirectorsDAO:
    ...
    def director_2(self, db):
        d = Director(name="test_name_2")
        db.session.add(d)
        db.session.commit()
        return d

    def test_get_director_by_id(self, director_2, directors_dao):
        assert directors_dao.get_by_id(director_2.id) == director_2

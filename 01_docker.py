...
записываем шаг для доступа в DockerHub:
--------------------------------------     
...
Теперь открываем DockerHub, переходим в репо 'coursework_4' и видим, что появился тэг с ID
...
services:
  api:
    image: armenar/coursework_4:$GITHUB_REF_NAME-$GITHUB_RUN_ID
    ports:
      - 80:80
    volumes:
      - ./docker_config.py:/code/default_config.py
  ...
  pg:
    ...
    healthcheck:
      ...
      retries: 5
--------------------------------------------------
для локального запуска команды envsubst нужно установить дополнительный пакет(не обязательно делать)
...
Нужно задать значения:
В терминале PyCharm
находясь в папке проекта
'export GITHUB_REF_NAME=main'
'export GITHUB_RUN_ID=1'
Повторно запустив команду 'type docker-compose-ci.yaml | envsubst > docker-compose-test.yaml' заметим, что в
файле docker-compose-test.yaml появились значения.
Таким образом можно создать шаблон docker-compose-ci.yaml и с помощью envsubst задавать разные значения.

Для добавления нового job дописываем файл action.yaml
-----------------------------------------------------
...
jobs:
  build_and_push:
    ...
    steps:
      ...
      - name: docker build
        run: docker build -t armenar/coursework_4:$GITHUB_REF_NAME-$GITHUB_RUN_ID .
      - name: docker login
        run: echo ${{ secrets.DOCKER_TOKEN }} | docker login -u ${{ secrets.DOCKER_USERNAME }} --password-stdin
      - name: docker push
        run: docker push armenar/coursework_4:$GITHUB_REF_NAME-$GITHUB_RUN_ID
  deploy:  # новый job
    runs-on: ubuntu-latest
    steps:  # шаг - подготовка файлов для закачки их на сервер
      - name: clone code  # копирует код
        uses: actions/checkout@v2  # для копирования используем команду actions/checkout@v2 
      - name: render configs  # 
        run: |  # запускаем run и можем в одном шаге можем передавать несколько команд
          mkdir deploy  # создание папки
          cat docker-compose-ci.yaml | envsubst > deploy/docker-compose.yaml  # печать содержимого файла и передаём
          с новыми значениями в новый файл во вновь созданной папке
 Тоже самое делается с файлом docker_config.py(в проекте coursework_4 нет его)
 -----------------------------------------------------------------------------
копируем этот файл и сохраняем его по наименованию docker_cia_config.py
внутри вместо наименования пользователя flask_app_user записываем $DB_USER, вместо flask_app_password записываем
$DB_PASSWORD и вместо flask_app_db записываем $DB_NAME. Получится: 
SQLALCHEMY_DATABASE_URI = "postgresql://$DB_USER:$DB_PASSWORDd@pg/$DB_NAME"     
И этот шаг добавляем в run:
name: Build and deploy workflow
on: [push]
jobs:
  build_and_push:
  ...
  deploy:  # новый job
    runs-on: ubuntu-latest
    needs: build_and_push  # команда задаёт очерёдность выполнения(сначала build_and_push, а потом deploy)
    steps:  # шаг - подготовка файлов для закачки их на сервер
      - name: clone code  # копирует код
        uses: actions/checkout@v2  # для копирования используем команду actions/checkout@v2 
      - name: render configs  # 
        run: |  # запускаем run в одном шаге можем передавать несколько команд символом '|'
          mkdir deploy  # создание папки
          cat docker-compose-ci.yaml | envsubst > deploy/docker-compose.yaml

Для отправки подготовленных файлов пишем следующий код: 
------------------------------------------------------   
Заходим в GitHub в чёрном сверху во вкладку Marketplace в разделе Types слева выбираем Actions, здесь все actions
(к примеру Checkout создан официальными разработчиками GitHub этот экшен для копирования файлов). Можно посмотреть
и actions SCP для файлов: 
      - name: copy files to server  #
        uses: appleboy/scp-action@master  # из Marketplace для файлов
        with:
          host: ${{ secrets.HOST }}  # скрывает хост
          username: ${{ secrets.USERNAME }}  # скрывает пользователя
          password: ${{ secrets.PASSWORD }}  # скрывает пароль
          source: "deploy/docker-compose.yaml"  # куда скачивать
          target: "coursework_4_cia"  # куда складывать файлы в виртуальной машине
          strip_components: 1  # эта команда позволяет при выводе пути файла не писать впереди слешов папки(если 1
          значит перед файлом один слэш, 2 - два слеша и т.д.)

Для указания значений секретов HOST, USERNAME и PASSWORD подключаемся к виртуальной машине из терминала сервера
и создаём специального пользователя, с помощью которого будем деплоить приложение:
В терминале сервера:
'ssh araru@ararutyun.ga'
'sudo su'
'adduser deploy'  # создаём нового пользователя по имени deploy
вводим пароль два раза и через 3 Enther и Y создаём пользователя
Теперь этого нового пользователя добавляем в группу админов (это для запуска docker-compose нужен администраторски
права)
'usermod -aG sudo deploy'  # добавление пользователя в группу админов
После этого нужно отредактировать конфиг ssh:
'vim /etc/ssh/ssh_config'
В этом файле находим строчку, запрещающую авторизацию по ssh через пароль и заменить с 'no' на 'yes'
И перезапускаем демон, отвечающий за 'ssh'
'service ssh reload'
и можем войти под другим именем пользователя:
'ssh deploy@ararutyun.ga' 
ввод пароля
'sudo su'
ввод пароля
root@armen1:/home/deploy# подключаемся как администратор

Для сохранения серкетов переходим в GitHub в репозиторий, Setting, secret значения для HOST, USERNAME и PASSWORD как
выше записывали для докеров, только пароль без токена, напрямую пароль пишем.
Переходим в PyCharm и в терминале:
'git add .'  # добавляем изменения
'git commit -m "transfer files"'  # коммитим
'git push'  # и пушим 

Перейдя в GitHub увидим новый репозиторий с переданными значениями HOST, USERNAME и PASSWORD
...
root@armen1:/home/deploy# подключаемся как администратор
'ls' содержимое
Видим папку coursework_4_cia
Для передачи значений в pg файла docker-compose.yaml (переменные окружения)POSTGRES_USER, POSTGRES_PASSWORD,
POSTGRES_DB дописываем код в файле action.yaml внутри job-а deploy с помощью секции env:
    ...
    steps:
      ...
      - name: render configs
        run: |
          mkdir deploy
          cat docker-compose-ci.yaml | envsubst > deploy/docker-compose.yaml
          cat docker-ci-config.py | envsubst > deploy/docker-config.py  # в проекте coursework_4 не нужно
      - name: copy files to server
